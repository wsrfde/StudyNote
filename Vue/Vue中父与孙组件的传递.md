...
当在父组件向子组件传递数据时，并且子组件不声明props，则$attrs包含所有属性的集合。并且会在子组件的根标签中声明被传递的属性。如：`<div name='vicer'></div>`
...
当在父组件向子组件传递数据时，并且子组件不声明props，默认在

但从父级传递过来的属性子组件不可能完全都用到，有一部分可能要传给孙组件，这时就要在子组件中声明`inheritAttrs=false`，不继承属性，并通过`v-bind=“$attrs"`传递给孙组件





上述特性的使用,完全可以降低在不使用Vuex以及Vue bus的情况下，组件跨级传递复杂度。